...
describe("functions", () => {
  ...
  describe(saveToFile, () => {
    ...
    it("Calls the saving with the correct params", () => {
      console.log = jest.fn();
      const language = MockResultObject.toString();

      saveToFile(language, "en");

      const json = JSON.stringify(language);
      const path = "./output/en.json";

      expect(fs.writeFile).toBeCalledWith(path, json, "utf8", undefined);
    });
  });
});
